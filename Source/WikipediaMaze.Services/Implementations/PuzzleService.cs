    ...
    public sealed class PuzzleService : IPuzzleService
    {
        ...
        public int UpdatePuzzleVoteCount(int puzzleId)
        {
            ...
            using (var session = _repository.OpenSession())
            {
                ...
                {
                    puzzleVoteCount =
                        _repository.All<Vote>().Where(p => p.PuzzleId == puzzleId).Sum(p => (int) p.VoteType);
        ...
        public IEnumerable<Vote> GetVotes(IEnumerable<Puzzle> puzzles, int userId)
        {
            ...
            using (_repository.OpenSession())
            {
                ...
                {
                    var vote = _repository.All<Vote>().Where(x => x.PuzzleId == puzzle.Id
                                                             && x.UserId == userId).OrderByDescending(x => x.DateVoted)
                                                             .FirstOrDefault();
        ...
        public IEnumerable<Vote> GetVotes(IEnumerable<PuzzleDetailView> puzzles, int userId)
        {
            ...
            using (_repository.OpenSession())
            {
                foreach (var puzzle in puzzles)
                {
                    var vote = _repository.All<Vote>().Where(x => x.PuzzleId == puzzle.PuzzleId
                                                             && x.UserId == userId).OrderByDescending(x => x.DateVoted)
                                                             .FirstOrDefault();
        ...
        public VoteResult VoteOnPuzzle(int puzzleId, VoteType voteType)
        {
            ...
            using (_repository.OpenSession())
            {
                user = _repository.All<User>().ById(_authenticationService.CurrentUserId);
