...
namespace WikipediaMaze.Web.Controllers
{
    ...
    public class FeedsController : Controller
    {
        ...
        public ActionResult PlayerSolutions(int id, string userName, SolutionSortType? sortType, FeedFormat? format)
        {
            ...
            IPagination<SolutionProfile> solutions = _puzzleService.GetSolutionsByUserId(id, sortType.Value, 1, 10000);

            return format.Value == FeedFormat.Json ?
                GetSolutionJsonFeed(solutions) :
                GetSolutionRssFeed(sortType, format, player, solutions);
        }
        private ActionResult GetSolutionJsonFeed(IEnumerable<SolutionProfile> solutions)
        {
            var jsonSolutions =
                solutions.Select(
                    x =>
        ...
        private ActionResult GetSolutionRssFeed(SolutionSortType? sortType, FeedFormat? format, User player, IEnumerable<SolutionProfile> solutions)
        {
        ...
        private static SyndicationItem GetSyndicationItem(SolutionProfile solution, User player)
        {
        ...
        private static string GetSolutionSummary(SolutionProfile solution)
        {
            return
                @"
                <table>
