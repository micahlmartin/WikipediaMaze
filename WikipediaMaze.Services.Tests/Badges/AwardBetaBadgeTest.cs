...
{
    [TestFixture]
    public class AwardBetaBadgeTest
    {
        [Test]
        public void DoesNotAllowMultiples()
        {
            ...
            var awarder = new AwardBetaBadge();
        ...
        [Test]
        public void DoesNotAssignBadgeForUsersCreatedAfterCuttofDate()
        {
            var moqer = new AutoMoqer();
            moqer.GetMock<IRepository>().Setup(x => x.All<User>()).Returns(new List<User>{new User
                                                                                       {
                                                                                           Id = 1,
                                                                                           Badges = new List<UserBadgeInfo>(),
                                                                                           DateCreated = new DateTime(2010,3,1,0,0,0, DateTimeKind.Utc)
                                                                                       }}.AsQueryable());
        ...
        public void AssignsBadge()
        {
            ...
            moqer.GetMock<IRepository>().Setup(x => x.All<User>()).Returns(new List<User>{new User
                                                                                       {
                                                                                           Id = 1,
                                                                                           Badges = new List<UserBadgeInfo>(),
                                                                                           DateCreated = new DateTime(2010,2,28,0,0,0, DateTimeKind.Utc)
                                                                                       }}.AsQueryable());
            ...
            moqer.Verify<IRepository>(x => x.Save(It.Is<User>(user =>
                                                                      user.Badges.Any(
                                                                          y =>
                                                                          y.Count == 1 &&
                                                                          y.Name == BadgeType.Beta.ToString())
                                                                  )), Times.Once());
        }
    }
}
